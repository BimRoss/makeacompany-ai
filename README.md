...
| `BACKEND_INTERNAL_SERVICE_TOKEN` | Optional bearer for `/v1/internal/*` snapshot refresh (CronJobs). **Not** Google OAuth; unset locally if you only refresh via signed-in `/admin` proxies (backend then accepts admin session Bearer on those POSTs). |
