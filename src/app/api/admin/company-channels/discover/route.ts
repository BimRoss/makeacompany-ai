...
export async function POST(req: Request) {
  ...
  const backendURL = `${resolveBackendBaseURL().replace(/\/$/, "")}/v1/admin/company-channels/discover`;
  try {
    ...
    return NextResponse.json({ error: `company-channels discover proxy failed: ${message}` }, { status: 502 });
  }
}
