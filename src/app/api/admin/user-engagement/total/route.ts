...
export async function GET() {
  const backendURL = `${resolveBackendBaseURL().replace(/\/$/, "")}/v1/admin/user-engagement/total`;
  try {
    ...
    return adminProxyNextJson({ error: `user-engagement/total proxy failed: ${message}` }, 502);
  }
}
