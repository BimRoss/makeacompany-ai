...
export function MePersonalAgentEditModal({
  ...
  // Reset to fresh values whenever the modal is reopened.
  useEffect(() => {
    if (!open) return;
    setName(initialName);
  ...
  return (
    ...
    >
      ...
      >
        <header className="flex items-center justify-between border-b border-border/60 bg-muted/20 px-5 py-4">
          <h2 id="edit-agent-modal-title" className="text-base font-semibold tracking-tight text-foreground">
            Edit personal agent
          </h2>
          <button
            type="button"
        ...
        <form onSubmit={save} className="space-y-4 px-5 py-5">
          ...
          <div className="flex justify-end gap-2 border-t border-border/60 pt-4">
            ...
            <button
              type="submit"
              disabled={submitting}
              className="inline-flex h-9 items-center justify-center rounded-lg bg-foreground px-4 text-xs font-semibold text-background transition hover:bg-foreground/90 disabled:cursor-not-allowed disabled:opacity-50"
            >
