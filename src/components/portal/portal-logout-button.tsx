...
export function PortalLogoutButton({ channelId }: Props) {
  ...
  async function onClick() {
    ...
    setError(null);
    try {
      const res = await fetch("/api/portal/auth/logout", { method: "POST", cache: "no-store" });
      if (!res.ok) {
      ...
      window.location.assign(`/${ch}/login`);
    } catch {
